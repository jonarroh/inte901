...
export class InventarioPostresComponent {
  ...
  constructor(private productoService: ProductoService) {
    ...
    this.inventarioPostresSource$ = this.inventarioPostresService
      .getInventarioPostre()
      .pipe(
        map((inventarioPostres) =>
          inventarioPostres.filter(
            (inventarioPostre) => inventarioPostre.estatus === 1
          )
        )
      );
    ...
    this.inventarioPostresSource$
      .pipe(
        ...
        map(([inventarioPostres, filterValue]) => {
          ...
          const filteredInventarioPostres = inventarioPostres.filter(
            (inventarioPostre) =>
              inventarioPostre.idProducto
                ?.toString()
                .includes(filterValue.toLowerCase()) ?? false
  ...
  onSubmitAdd(form: NgForm) {
    if (form.valid) {
      ...
      this.inventarioPostresService
        .registrarInventarioPostre(this.inventarioPostre)
        .subscribe({
          next: (response) => {
  ...
  onSubmitEdit(form: NgForm) {
    if (form.valid) {
      ...
      // });
      this.inventarioPostresService
        .editarInventarioPostre(
          this.inventarioPostre.idPostre!,
          this.inventarioPostre
        )
        .subscribe({
          next: (response) => {
            console.log('Inventario de postre actualizado:', response);
          ...
          error: (error) => {
            console.error(
              'Error al actualizar el inventario de postre:',
              error
            );
